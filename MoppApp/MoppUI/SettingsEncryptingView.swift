...
public struct SettingsEncryptingView: View, Initializable {
    ...
    private var x509: X509Certificate? {
        guard let data = cert else { return nil }
        ...
        Form {
            ...
            if enableCDoc2Encryption {
                ...
                if enableCDoc2OnlineEncryption {
                    ...
                    if selected == model.items.last?.id {
                        Text("Issued to: \(x509?.subject(oid: .commonName)?.joined(separator: ", ") ?? "")")
                        Text("Valid to: \(x509?.notAfter?.formatted(.iso8601) ?? "")")
                        Button("ADD CERTIFICATE") {
                            isImporting.toggle()
                        }
                        .frame(maxWidth: .infinity, alignment: .center)
                        .fileImporter(isPresented: $isImporting, allowedContentTypes: [.x509Certificate]) { result in
                            switch result {
                            case .success(let url):
                                cert = try? Data(contentsOf: url)
                                if let pemString = String(data: cert ?? Data(), encoding: .utf8),
                                   pemString.contains("-----BEGIN CERTIFICATE-----") {
                                    cert = Data(base64Encoded: pemString
